...
<body class="font-sans text-slate-800">
    ...
    <div id="page-dashboard" class="page-section bg-slate-50 min-h-screen pt-10 px-4">
        <div class="max-container py-10">
            ...
            <div class="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-sm mb-8">
            ...
            <div class="bg-white p-4 sm:p-8 rounded-3xl border border-slate-200 shadow-sm">
                ...
                <div id="dash-nodes-grid" class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 xl:grid-cols-10 gap-3 sm:gap-4 max-h-[60vh] overflow-y-auto custom-scrollbar p-1"></div>
            </div>
        </div>
    </div>
    ...
    <div id="page-contact" class="page-section bg-slate-50 min-h-screen pt-10">
        <div class="max-container max-w-5xl py-10 px-4">
    ...
    <script>
        ...
        const dict = {
            th: {
                ...
                aboutVisionTitle: "Vision", aboutMissionTitle: "Mission", aboutDesc: "Snapcon Automation คือผู้นำเทคโนโลยีอุตสาหกรรมยุคใหม่ ที่เน้นความง่ายในการเชื่อมต่อและการติดตั้งในรูปแบบ Plug & Play System เรามุ่งมั่นที่จะพลิกโฉมวงการออโตเมชันด้วยโซลูชันที่ลดความซับซ้อน ลดเวลาในการติดตั้ง และเพิ่มประสิทธิภาพการผลิตสูงสุด",
                ...
                dashSubTitle: "ระบบตรวจสอบระดับองค์กรพร้อมระบบซ่อมบำรุงเชิงคาดการณ์", dashCtrlTitle: "System Controls", dashCfgTitle: "Configuration", dashTarget: "Target", dashCarbon: "Carbon Factor", dashEnergy: "Energy Factor",
            ...
            en: {
                ...
                pageProductTitle: "Conveyor Systems", pageSpareTitle: "Spare Parts", pageProjectTitle: "Project Reference", 
                projPilotTitle: "Pilot Project", projUseCaseTitle: "Use Case / Application",
                pageCartTitle: "Quotation Request", cartEmpty: "Your cart is empty", cartTotalLabel: "ESTIMATED TOTAL", btnRequestQuote: "SUBMIT REQUEST", selectAll: "Select All", deleteSelected: "Delete",
                guestContactTitle: "Contact Info", contactSub: "Official Technical Support & Inquiries, available 24/7.",
                aboutVisionTitle: "Vision", aboutMissionTitle: "Mission", aboutDesc: "Snapcon Automation is a leader in modern industrial technology, focusing on ease of connection and installation through Plug & Play Systems. We are committed to revolutionizing the automation industry with solutions that reduce complexity, save time, and maximize production efficiency.",
                ...
                dashSubTitle: "Enterprise Monitoring & Predictive Maintenance System", dashCtrlTitle: "Controls", dashCfgTitle: "Config", dashTarget: "Target", dashCarbon: "Carbon", dashEnergy: "Energy",
        ...
        function setLanguage(lang) {
            ...
            document.getElementById('btn-lang-th').className = lang === 'th' ? "text-xs font-bold text-snap-green" : "text-xs font-bold text-slate-400 hover:text-white transition-colors";
            document.getElementById('btn-lang-en').className = lang === 'en' ? "text-xs font-bold text-snap-green" : "text-xs font-bold text-slate-400 hover:text-white transition-colors";
            renderProducts(); renderProjects(); renderArticles(); renderDocuments();
        }
        ...
        function renderDocuments() {
            // 1. นำข้อมูลไปใส่ในปุ่ม (Dropdown) ใต้ Banner
            ...
            const dsMenu = document.getElementById('menu-datasheet'); if(dsMenu) dsMenu.innerHTML = makeMenu('datasheet');
            const dwMenu = document.getElementById('menu-drawing'); if(dwMenu) dwMenu.innerHTML = makeMenu('drawing');
            const caMenu = document.getElementById('menu-catalog'); if(caMenu) caMenu.innerHTML = makeMenu('catalog');

            // 2. นำข้อมูลไปใส่ในกล่อง Tab ด้านล่าง
            const makeList = (typeStr) => {
                ...
                return f.map(d => {
                    const n = d.modelname || d.name || d.model || d.title || 'Untitled';
                    const u = d.fileurl || d.link || d.url || d.file || '#';
                    if(u==='#') return '';
                    let icon = 'fa-file';
            ...
            const dsContent = document.getElementById('content-datasheet'); if(dsContent) dsContent.innerHTML = makeList('datasheet');
            const dwContent = document.getElementById('content-drawing'); if(dwContent) dwContent.innerHTML = makeList('drawing');
            const caContent = document.getElementById('content-catalog'); if(caContent) caContent.innerHTML = makeList('catalog');
        }
